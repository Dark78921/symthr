...
contract Issuer is Owned, MixinSystemSettings, IIssuer {
    ...
    function _maxIssuableSynths(address _issuer) internal view returns (uint256, bool) {
        // What is the value of their SNX balance in sUSD
        (uint256 snxRate, bool isInvalid) = exchangeRates().rateAndInvalid(SNX);
        uint256 destinationValue = _snxToUSD(_collateral(_issuer), snxRate);
    ...
    function _voluntaryBurnSynths(
        ...
        (uint256 maxIssuableSynthsForAccount, bool snxRateInvalid) = _maxIssuableSynths(from);
        _requireRatesNotInvalid(anyRateIsInvalid || snxRateInvalid);
