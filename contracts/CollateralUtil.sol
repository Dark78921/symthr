...
contract CollateralUtil is ICollateralUtil, ICollateralLoan, MixinSystemSettings {
    ...
    function getCollateralRatio(Loan calldata loan, bytes32 collateralKey) external view returns (uint256 cratio) {
        uint256 cvalue = _exchangeRates().effectiveValue(collateralKey, loan.collateral, sUSD);
        uint256 dvalue = _exchangeRates().effectiveValue(loan.currency, loan.amount.add(loan.accruedInterest), sUSD);
    ...
    function maxLoan(
        ...
        uint256 ratio = SafeDecimalMath.unit().divideDecimalRound(minCratio);
    ...
    function liquidationAmount(
        ...
        uint256 liquidationPenalty = getLiquidationPenalty();
        uint256 debtValue = _exchangeRates().effectiveValue(loan.currency, loan.amount.add(loan.accruedInterest), sUSD);
        uint256 collateralValue = _exchangeRates().effectiveValue(collateralKey, loan.collateral, sUSD);
        uint256 unit = SafeDecimalMath.unit();

        uint256 dividend = debtValue.sub(collateralValue.divideDecimal(minCratio));
        uint256 divisor = unit.sub(unit.add(liquidationPenalty).divideDecimal(minCratio));

        uint256 sUSDamount = dividend.divideDecimal(divisor);
