...
contract PerpsV2SettingsMixin is MixinResolver {
    ...
    function _baseFee(bytes32 _marketKey) internal view returns (uint256) {
        return _parameter(_marketKey, PARAMETER_BASE_FEE);
    }

    function _baseFeeNextPrice(bytes32 _marketKey) internal view returns (uint256) {
    ...
    function _maxSingleSideValueUSD(bytes32 _marketKey) internal view returns (uint256) {
