...
interface IFuturesMarketManagerInternal {
    ...
    function payFee(uint256 amount, bytes32 trackingCode) external;
}
...
contract PerpsV2MarketBase is PerpsV2SettingsMixin, IPerpsV2BaseTypes {
    ...
    mapping(uint256 => address) public positionIdOwner;
    ...
    function _orderSizeTooLarge(
        ...
        int256 newMarketSize = int256(marketSize).sub(_signedAbs(oldSize)).add(_signedAbs(newSize));
    ...
    function _profitLoss(Position memory position, uint256 price) internal pure returns (int256 pnl) {
        int256 priceShift = int256(price).sub(int256(position.lastPrice));
    ...
    function _marginPlusProfitFunding(Position memory position, uint256 price) internal view returns (int256) {
        ...
        return int256(position.margin).add(_profitLoss(position, price)).add(funding);
    }
    ...
    function _orderFee(TradeParams memory params, uint256 dynamicFeeRate) internal pure returns (uint256 fee) {
        // usd value of the difference in position
        int256 notionalDiff = params.sizeDelta.multiplyDecimal(int256(params.price));

        uint256 feeRate = params.baseFee.add(dynamicFeeRate);
        ...
        )
    {
        ...
        if (
            _orderSizeTooLarge(
                uint256(int256(_maxSingleSideValueUSD(marketKey).add(100 * uint256(_UNIT))).divideDecimal(int256(params.price))),
                oldPos.size,
                newPos.size
            )
        ) {
     ...
     */
    function _assetPriceRequireSystemChecks(bool allowMarketPaused) internal returns (uint256) {
        ...
        (uint256 price, bool invalid) = _exchangeCircuitBreaker().rateWithInvalid(baseAsset);
    ...
    function _positionDebtCorrection(Position memory position) internal view returns (int256) {
         ...
         */
        return
            int256(position.margin).sub(
                int256(position.size).multiplyDecimal(int256(position.lastPrice).add(fundingSequence[position.lastFundingIndex]))
            );
    }
    ...
    function transferMargin(int256 marginDelta) external {
        ...
        uint256 price = _assetPriceRequireSystemChecks(allowMarketPaused);
    ...
    event Tracking(bytes32 indexed trackingCode, bytes32 baseAsset, bytes32 marketKey, int256 sizeDelta, uint256 fee);
}
