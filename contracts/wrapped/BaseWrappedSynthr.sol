...
contract BaseWrappedSynthr is IERC20, ExternStateToken, MixinResolver, ISynthetix {
    // ========== STATE VARIABLES ==========
    ...
    address[] public availableCollateralCurrencies;
    ...
    // function _canTransfer(address account, uint256 value) internal view returns (bool) {
    //     if (issuer().debtBalanceOf(account, sUSD) > 0) {
    //         (uint256 transferable, bool anyRateIsInvalid) = issuer().transferableSynthetixAndAnyRateIsInvalid(
    //             account,
    ...
    function addCollateralCurrency(address _collateralAddress, bytes32 _currencyKey) external optionalProxy {
        ...
        availableCollateralCurrencies.push(_collateralAddress);
    ...
    function transfer(address to, uint256 value) external onlyProxyOrInternal systemActive returns (bool) {
    ...
    function issueSynthsOnBehalf(address issueForAddress, uint256 amount) external issuanceActive optionalProxy {
    ...
    function liquidateDelinquentAccount(address account) external systemActive optionalProxy returns (bool) {
        (uint256 totalRedeemed, uint256 amountLiquidated) = issuer().liquidateAccount(account, false);
    ...
    function liquidateSelf() external systemActive optionalProxy returns (bool) {
        ...
        (uint256 totalRedeemed, uint256 amountLiquidated) = issuer().liquidateAccount(messageSender, true);
    ...
    function initializeLiquidatorRewardsRestitution(uint256 amount) external onlyOwner {
        if (!restituted) {
            restituted = true;
