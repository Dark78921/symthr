...
contract SynthrIssuer is Owned, MixinSystemSettings, IIssuer {
    ...
    {
        (, int256 rawIssuedSynths, , uint256 issuedSynthsUpdatedAt, ) = AggregatorV2V3Interface(
            requireAndGetAddress(CONTRACT_EXT_AGGREGATOR_ISSUED_SYNTHS)
        ).latestRoundData();

        (, int256 rawRatio, , uint256 ratioUpdatedAt, ) = AggregatorV2V3Interface(
            requireAndGetAddress(CONTRACT_EXT_AGGREGATOR_DEBT_RATIO)
        ).latestRoundData();

        debt = uint256(rawIssuedSynths);
    ...
    function _sharesForDebt(uint256 debtAmount) internal view returns (uint256) {
        (, int256 rawRatio, , , ) = AggregatorV2V3Interface(requireAndGetAddress(CONTRACT_EXT_AGGREGATOR_DEBT_RATIO))
            .latestRoundData();

        return rawRatio == 0 ? 0 : debtAmount.divideDecimalRoundPrecise(uint256(rawRatio));
    }
    ...
    function _debtForShares(uint256 sharesAmount) internal view returns (uint256) {
        ...
        return sharesAmount.multiplyDecimalRoundPrecise(uint256(rawRatio));
    }
    ...
    {
        (uint256 debt, , bool cacheIsInvalid, bool cacheIsStale) = debtCache().cacheInfo();
        ...
        if (!excludeCollateral) {
            (uint256 nonSnxDebt, bool invalid) = debtCache().totalNonSnxBackedDebt();
        ...
        (uint256 currencyRate, bool currencyRateInvalid) = exRates.rateAndInvalid(currencyKey);
    ...
    function _snxToUSD(uint256 amount, uint256 snxRate) internal pure returns (uint256) {
        return amount.multiplyDecimalRound(snxRate);
    }

    function _usdToSnx(uint256 amount, uint256 snxRate) internal pure returns (uint256) {
    ...
    function _collateralisationRatio(address _issuer) internal view returns (uint256, bool) {
        ...
        (uint256 debtBalance, , bool anyRateIsInvalid) = _debtBalanceOfAndTotalDebt(synthetixDebtShare().balanceOf(_issuer), SNX);
    ...
    function totalIssuedSynths(bytes32 currencyKey, bool excludeOtherCollateral) external view returns (uint256 totalIssued) {
    ...
    function debtBalanceOf(address _issuer, bytes32 currencyKey) external view returns (uint256 debtBalance) {
        ...
        uint256 debtShareBalance = sds.balanceOf(_issuer);
    ...
    function issuableSynthExpected(
        ...
        uint256 totalCollateralExpected = currentCollateral + synthrToMint;

        // They're allowed to issue up to issuanceRatio of that value
        maxIssuable = totalCollateralExpected.multiplyDecimal(getIssuanceRatio());
    ...
    function checkFreeCollateral(
        ...
        (uint256 alreadyIssued, , ) = _debtBalanceOfAndTotalDebt(synthetixDebtShare().balanceOf(_issuer), sUSD);
        uint256 lockedSynthrValue = alreadyIssued.divideDecimalRound(getIssuanceRatio());
        uint256 currentCollateral = _collateral(_issuer);
        if (lockedSynthrValue > currentCollateral) {
            withdrawableSynthr = 0;
        } else {
            (uint256 collateralRate, ) = exchangeRates().rateAndInvalid(_collateralKey);
            uint256 freeCollateral = currentCollateral - lockedSynthrValue;
            withdrawableSynthr = freeCollateral.divideDecimalRound(collateralRate);
        }
    }
    ...
    //     uint256 lockedSynthetixValue = debtBalance.divideDecimalRound(getIssuanceRatio());
    ...
    function issueSynthsWithoutDebt(
        ...
        (uint256 rate, bool rateInvalid) = exchangeRates().rateAndInvalid(currencyKey);
    ...
    function issueSynths(address from, uint256 amount) external onlySynthetix {
        require(amount > 0, "Issuer: cannot issue 0 synths");
    ...
    function burnSynths(address from, uint256 amount) external onlySynthetix {
        ...
        returns (uint256 totalRedeemed, uint256 amountToLiquidate)
    {
        ...
        (uint256 debtBalance, , bool anyRateIsInvalid) = _debtBalanceOfAndTotalDebt(
            synthetixDebtShare().balanceOf(account),
            sUSD
        );
    ...
    function _issueSynths(
        ...
        if (!_verifyCircuitBreaker()) {
            // TODO debt aggregator usage
            return;
        }

        (uint256 maxIssuable, , , bool anyRateIsInvalid) = _remainingIssuableSynths(from); // TODO debt Aggregator and total issued synth aggregator usage
        ...
        _addToDebtRegister(from, amount); // TODO liquidatorRewards usage
    ...
    function _voluntaryBurnSynths(
        ...
        if (!burnToTarget) {
            ...
            (, uint256 refunded, uint256 numEntriesSettled) = exchanger().settle(from, sUSD);
        ...
        uint256 amountBurnt = _burnSynths(from, from, amount, existingDebt);
    ...
    function _addToDebtRegister(address from, uint256 amount) internal {
    ...
    function _verifyCircuitBreaker() internal returns (bool) {
        ...
        uint256 deviation = _calculateDeviation(lastDebtRatio, uint256(rawRatio));
