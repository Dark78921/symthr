...
contract Synthetix is BaseSynthetix {
    ...
    function mint() external issuanceActive returns (bool) {
        ...
        uint256 supplyToMint = _supplySchedule.mintableSupply();
        require(supplyToMint > 0, "No supply is mintable");

        emitTransfer(address(0), address(this), supplyToMint);

        // record minting event before mutation to token supply
        uint256 minterReward = _supplySchedule.recordMintEvent(supplyToMint);

        // Set minted SNX balance to RewardEscrow's balance
        // Minus the minterReward and set balance of minter to add reward
        uint256 amountToDistribute = supplyToMint.sub(minterReward);
