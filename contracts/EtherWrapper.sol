...
contract EtherWrapper is Owned, Pausable, MixinResolver, MixinSystemSettings, IEtherWrapper {
    ...
    uint256 public sETHIssued = 0;
    uint256 public sUSDIssued = 0;
    uint256 public feesEscrowed = 0;
    ...
    function mint(uint256 amountIn) external notPaused {
    ...
    function burn(uint256 amountIn) external notPaused {
        ...
        uint256 principal = amountIn.divideDecimalRound(SafeDecimalMath.unit().add(burnFeeRate()));
    ...
    function distributeFees() external {
        ...
        uint256 amountSUSD = exchangeRates().effectiveValue(sETH, feesEscrowed, sUSD);
    ...
    function _mint(uint256 amountIn) internal {
        // Calculate minting fee.
        uint256 feeAmountEth = calculateMintFee(amountIn);
        uint256 principal = amountIn.sub(feeAmountEth);
    ...
    function _burn(uint256 principal, uint256 amountIn) internal {
        // for burn, amount is inclusive of the fee.
        uint256 feeAmountEth = amountIn.sub(principal);
